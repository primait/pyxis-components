module Prima.PyxisV3.Table exposing
...
render (State ({ sortBy, sortedColumn } as internalState)) (Config ({ headers, rows, footerColumns, skin } as conf)) =
    ...
    table
        [ classList
            ([ ( "table", True )
             , ( "table--base", isBase skin )
             , ( "table--light", isLight skin )
             , ( "table--alternate-rows", options.alternateRows )
             ]
...
renderTHead internalState ({ headers } as conf) options =
    thead
        [ classList (( "table__header", True ) :: options.headerClassList)
        ]
        [ tr
            [ class "table__header__row" ]
...
renderTFoot headerSlugs options footerRows =
    tbody
        [ classList (( "table__footer", True ) :: options.footerClassList) ]
...
renderSortIcon isSorted algorithm =
    if isSorted then
        i
            [ classList
                [ ( "table__header__row__item__icon", True )
                , ( "icon", True )
                , ( "icon-caret-up", algorithm == Just Asc )
                , ( "icon-caret-down", algorithm == Just Desc )
                ]
            ]
            []
...
renderTD options ( slug, Column conf ) =
    td
        [ classList (( "table__body__row__item", True ) :: options.elementClassList)
...
renderFooterTD ( slug, Column conf ) =
    td
        [ class "table__footer__row__item"
        , (colspan << pickColSpan) conf
        , columnNameAttribute slug
        ]
        (case conf of
